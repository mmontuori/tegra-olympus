...
NvBool
Adt7461Therm2Asserted(
    NvOdmTmonDeviceHandle hTmon)
{
    ADT7461PrivData* pPrivData;
    NvU32 PinValue = 1; /*not asserted state */

    NV_ASSERT(hTmon && hTmon->pPrivate);

    pPrivData = hTmon->pPrivate;

    NvOdmGpioGetState(pPrivData->hGpio, pPrivData->hGpioPin, &PinValue);

    return PinValue ? NV_FALSE : NV_TRUE;   /* active low */
}
...
NvBool Adt7461Init(NvOdmTmonDeviceHandle hTmon)
{
     ...
     * run mode and re-start temperature monitoring
     */
    ...
    if(!Adt7461WriteReg(pPrivData, pReg, Data))
        goto fail;
    Data = ADT7461_T_VALUE_TO_DATA(
        ExtRange, ADT7461_ODM_REMOTE_THROTTLING_LIMIT_VALUE); 
    pReg = &pPrivData->pDeviceInfo->Channels[
        ADT7461ChannelID_Remote].IntrLimitHigh;
    if(!Adt7461WriteReg(pPrivData, pReg, Data))
        goto fail;

    // Program THERM hysteresis value.  It applies to both remote and local channels
    // and to both THERM (h/w shutdown) and THERM2 (throttling interrupt) outputs
    Data = ADT7461_ODM_THERM_HYSTERESIS;
    pReg = &pPrivData->pDeviceInfo->ComparatorHysteresis;
    if(!Adt7461WriteReg(pPrivData, pReg, Data))
        goto fail;
    ...
    NvOdmTmonParameterCaps* pCaps)
{
    ...
    switch (ParamId)
    {
        case NvOdmTmonConfigParam_DttIntrLimit:
            pCaps->OdmProtected =
